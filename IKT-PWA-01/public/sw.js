importScripts('/src/js/idb.js'); // stimmt die Variante, die ich habe?
...
const CACHE_VERSION = 84; //immer nach jeder wichtigen Änderung, bspw. in feed.js oder index.html
    ...
    event.waitUntil(
        caches.open(CURRENT_STATIC_CACHE)
            .then( cache => {
                ...
                cache.addAll([ //in die Caches laden
                    '/',
                    '/index.html',
                    '/src/js/app.js',
                    '/src/js/feed.js',
...
self.addEventListener('activate', event => {
    console.log('service worker --> activating ...', event); // steht in der Konsole
    event.waitUntil(
        caches.keys()
            .then( keyList => {
